...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def loop(count:Int, x:Int, y :Int): Int ={
      if(count == 0) x
      else loop (count - 1, y, x + y)
    }
    loop(n, 0, 1)
  }
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    @annotation.tailrec
    def loop(n :Int, bSorted :Boolean): Boolean = {
      if (as.length <= n+1) bSorted
      else {
        val result = gt(as(n), as(n + 1))
        if(result) loop(n + 1, result)
        else result
      }
    }
    loop(0, false)
  ...
  */
  // CU: cheat answer is g andThen f
